...
public class HandEvaluatorFive {
    ...
    private List<Byte> unSortedCardValues = new ArrayList<>(); // Added myself

    public HandEvaluatorFive(CardVisual cards[]) {
        this.cards = cards;
        evaluateHand();
    }
    ...
    private void evaluateHand() {
        ...
        for (int i = 0; i < cards.length; i++) { // Original < 5 as length
        ...
        List<Byte> sortedCardValues = new ArrayList<>();

        for (byte i = 0; i < cards.length; i++) { // Original < 5 as length
            sortedCardValues.add(cards[i].getRank());
            unSortedCardValues.add(cards[i].getRank());
        }
